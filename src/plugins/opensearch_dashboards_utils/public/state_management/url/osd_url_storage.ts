 ...
 *
 * given an url:
 * http://localhost:5601/oxf/app/opensearch-dashboards#/yourApp?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')
 * will return object:
 ...
 * {tab: 'indexedFields'}
 */
export function getStateFromOsdUrl<State>(
  key: string,
  url: string = window.location.href
): State | null {
  return (getStatesFromOsdUrl(url, [key])[key] as State) || null;
}
 ...
 *
 * e.g.:
 * given a url: http://localhost:5601/oxf/app/opensearch-dashboards#/yourApp?_a=(tab:indexedFields)&_b=(f:test,i:'',l:'')
 * key: '_a'
 * and state: {tab: 'other'}
 *
 * will return url:
 ...
 */
export function setStateToOsdUrl<State>(
  key: string,
  state: State,
 ...
 */
export interface IOsdUrlControls {
  /**
...
export const createOsdUrlControls = (
